...
function customAssign(...objects) {
	...
	const mergetObjects = Object.assign({}, ...objects);
	for (const key in mergetObjects) {
		if (typeof mergetObjects[key] === "number") {
			mergetObjects[key] == String(mergetObjects[key]);
		}
	}

	return mergetObjects;
}
...
function customEntries(obj) {
	...
	const customEn = Object.entries(obj);
	return customEn;
}
...
function filterObjectsByKey(arr, key) {
	...
	const filteredObjects = arr.filter((obj) => obj.hasOwnProperty(key));
	return arr;
}
